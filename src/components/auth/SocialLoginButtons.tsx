...
export default function SocialLoginButtons({ 
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      // OAuth redireciona automaticamente, não precisa fazer nada
      
    } catch (error: any) {
      console.error(`Erro no login ${provider}:`, error)
      alert(`Erro no login via ${provider}: ${error.message}`)
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="text-center">
        <p className="text-xs text-gray-500">
          {userType === 'oficina' 
            ? `Criando conta ${oficinaPlano === 'pro' ? 'PRO' : 'FREE'} para oficina`
            : 'Criando conta para motorista'
          }
        </p>
      </div>
    </div>
